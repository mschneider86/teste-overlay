...
export class OverlayComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      const firstInput = this.elementRef.nativeElement.querySelector('input, textarea, select');
      if (firstInput) {
        firstInput.focus();
      }
  ...
  onMouseMove(event: MouseEvent): void {
    ...
    this.currentPosition.x = Math.max(0, Math.min(newX, windowWidth - elementWidth));
    this.currentPosition.y = Math.max(0, Math.min(newY, windowHeight - elementHeight));
